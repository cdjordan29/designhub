...
<%@ page import="com.hub.models.RadfordRecommendedReading"%>
<%@ page import="com.hub.models.StudentRecommendedReading"%>
<%@ taglib prefix="sec"
	...
	<div class="flex-center flex-column"></div>
		...
		<div class="container-fluid">
			<div class="flex-center flex-column" style="height: 15vh">
				<h2>Radford Recommended Reading</h2>
				</br>
			</div>

			<div class="row">
				<%
					java.util.List<RadfordRecommendedReading> radfordRecommendeds = (java.util.ArrayList<RadfordRecommendedReading>) request.getAttribute("radfordRecommendeds");
				...
				<div class="container-fluid col-lg-3">
					...
					<div class="card card-cascade wider reverse my-4">
						...
						<div class="card-body">
							...
							<a href="<%=radfordRecommendeds.get(i).getBuyLink()%>"
								class="btn btn-danger px-6" target="_blank"> Buy </a> <a
								href="<%=radfordRecommendeds.get(i).getDownload()%>"
								class="btn btn-danger" target="_blank"> Download </a>

							<div>
								<form
									action="readingroom/rrthumbsUp/<%=radfordRecommendeds.get(i).getId()%>"
									method="POST">
									...
									<input type="hidden" name="${_csrf.parameterName}"
										value="${_csrf.token}" /> <span><%=radfordRecommendeds.get(i).getThumbsUp()%></span>
								</form>

								<form
									action="readingroom/rrthumbsDown/<%=radfordRecommendeds.get(i).getId()%>"
									method="POST">
									<button type="submit" class="btn btn-default px-6">
									...
									<!-- <input type="submit" value="-" /> -->
									<input type="hidden" name="${_csrf.parameterName}"
										value="${_csrf.token}" /> <span><%=radfordRecommendeds.get(i).getThumbsDown()%></span>
								</form>
							...
							<sec:authorize access="hasRole('admin') || hasRole('mod')">
								...
								<div class="modal fade" id="editRR<%=radfordRecommendeds.get(i).getId()%>" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
            						...
            						<div class="modal-body">
            							<form action="readingroom/updateRadfordReading/<%=radfordRecommendeds.get(i).getId()%>" method="POST">
								<div class="md-form">
								...
								<div class="md-form">
										<input type="text" name="buy_link" class="form-control" value="<%=radfordRecommendeds.get(i).getBuyLink()%>">
								...
								<form action="readingroom/deleteRadfordRecommendation"
									method="POST">
			...
			<div class="col-sm-3 text-center">
					...
					<button type="button" class="btn col-lg-12" data-toggle="modal" data-target="#radfordReading">
    					Add New Recommendation
					</button>
					<!-- Modal -->
					<div class="modal fade" id="radfordReading" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
    					<div class="modal-dialog" role="document">
        				<div class="modal-content">
            			<div class="modal-header">
            			...
            			<div class="modal-body">
                			<form action="readingroom/addRadfordReading" method="POST">
								<div class="md-form">
								...
								<div class="md-form">
										<input type="text" name="buy_link" class="form-control">
										<label for="buy_link">Buy Link</label>
									</div>
								<div class="md-form">
		...
		<div class="container-fluid">
			<div class="flex-center flex-column" style="height: 15vh">
				<h2>Student Recommended Reading</h2>
				</br>
			</div>

			<div class="row">
				<%
					java.util.List<StudentRecommendedReading> studentRecommendeds = (java.util.ArrayList<StudentRecommendedReading>)request.getAttribute("studentRecommendeds");
				...
				<div class="col-lg-3">
					...
					<div class="card card-cascade wider reverse my-4">
						...
						<div class="card-body">
							...
							<a href="<%=studentRecommendeds.get(i).getBuyLink()%>"
								class="btn btn-danger" target="_blank"> Buy </a>

							<div>
								<form
									action="readingroom/srthumbsUp/<%=studentRecommendeds.get(i).getId()%>"
									method="POST">
									<button type="submit" class="btn btn-default px-6">
										<i class="fa fa-thumbs-o-up" aria-hidden="true"></i>
									</button>
								...
								<form
									action="readingroom/srthumbsDown/<%=studentRecommendeds.get(i).getId()%>"
									method="POST">
									<button type="submit" class="btn btn-default px-6">
										<i class="fa fa-thumbs-o-down" aria-hidden="true"></i>
									</button>
							...
							<sec:authorize access="hasRole('admin') || hasRole('mod')">
								...
								<div class="modal fade" id="editSR<%=studentRecommendeds.get(i).getId()%>" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
            						...
            						<div class="modal-body">
            							<form action="readingroom/updateStudentReading/<%=studentRecommendeds.get(i).getId()%>" method="POST">
								<div class="md-form">
									<input type="text" name="title" class="form-control" value="<%=studentRecommendeds.get(i).getTitle()%>">
									<label for="title">Title</label>
								</div>
								<div class="md-form">
										<input type="text" name="buy_link" class="form-control" value="<%=studentRecommendeds.get(i).getBuyLink()%>">
										<label for="buy_link">Buy Link</label>
									</div>
								<div class="md-form">
								...
								<form action="readingroom/deleteStudentRecommendation"
									method="POST">
									<input type="hidden" name="id"
			...
			<div class="col-sm-3 text-center">
					...
					<sec:authorize access="hasRole('admin') || hasRole('mod')">
					<!-- Button trigger modal -->
					<button type="button" class="btn col-lg-12" data-toggle="modal" data-target="#studentReading">
    					Add New Recommendation
					</button>
					<!-- Modal -->
					<div class="modal fade" id="studentReading" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
    					<div class="modal-dialog" role="document">
        				<div class="modal-content">
            			<div class="modal-header">
                			<h5 class="modal-title" id="modalLabel">New Recommendation</h5>
            			...
            			<div class="modal-body">
                			<form action="readingroom/addStudentReading" method="POST">
								<div class="md-form">
									<input type="text" name="title" class="form-control">
									<label for="title">Title</label>
								</div>
